...
self.addEventListener("fetch", (e) => {
  ...
  e.respondWith(caches.open(CACHE).then(async (c) => {
    if (r.mode === "navigate" || r.destination === "document") {
      try { const res = await fetch(r); if (res && res.ok) c.put(r, res.clone()); return res; } catch (e) { const h = await c.match(r, { ignoreSearch: true }); return h || new Response("Offline", { status: 503 }); }
    }
